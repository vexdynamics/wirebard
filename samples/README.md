...
These are reference partials (borrowed structure from haladin's `wishes/`),
showing how config fragments compose into `/etc/wireguard/<network>.conf`.
Nothing here is compiled by wirebard — it's documentation you can copy into a
real project root.
...
`server.key`. `00-main.conf` + `10-alice.conf` + `20-baki-web01.conf` →
...
[Peer]                            # from 20-baki-web01.conf, verbatim
...
`{{PRIVATE_KEY}}` is the only placeholder: the caller (e.g. baki) generated
the keypair locally and substitutes its own private key. wirebard only ever
sees the public key.

## Variables (`#=`) — reused nearly verbatim from haladin
